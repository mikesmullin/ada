...
void main() {
    ...
    // ---- ear rec/fuse ------------------------------------------------------
    // 1 armed  2 start-clock  3 start-empty  4 finish-clock
    // 5 finish-empty  6 got-utterance  7 barge-in
    if (ear_phase > 0.5) {
        float p = ear_phase;
        ...
        float blink = mix(0.45 + 0.55 * step(0.5, fract(t * 1.6)), 1.0,
                          is_finish + is_got + is_barge + is_smiss + is_fmiss);
        blink = mix(0.55 + 0.45 * (0.5 + 0.5 * sin(t * 2.2)), blink, 1.0 - is_armed);
        blink = mix(blink, 0.55 + 0.45 * (0.5 + 0.5 * sin(t * 2.2)), is_armed);
        ...
        if (timed > 0.5) {
            float ang01 = fract(0.25 - th / TAU);
            float lit = step(ang01, max(ear_t, 0.0));
            col += rec_col * ringLine(r, 0.955, 0.014, aa) * lit * (0.50 + 0.55 * ear_t);
            float head = 1.0 - smoothstep(0.0, 0.03, abs(ang01 - ear_t));
            col += rec_col * ringLine(r, 0.955, 0.018, aa) * head * 1.5;
        }
        if (is_smiss + is_fmiss > 0.5) {
            // empty timeout: hollow ring, fuse gone
            col += rec_col * ringLine(r, 0.955, 0.010, aa) * 0.35;
        }
